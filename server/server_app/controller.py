...
@app.route('/generate-pdf1', methods=['POST'])
def generate_pdf1():
    import io
    data = request.json
...
def user_register():
    ...
    if request.method.__eq__('POST'):
        ...
        try:
            ...
            err_msg = 'Tên đăng nhập đã sử dụng'
...
def patient_information_redirect():
    if not current_user.is_authenticated:
        return redirect(url_for("login"))  # Chuyển đến trang đăng nhập nếu chưa đăng nhập
    return redirect(url_for("patient_information_user", user_id=current_user.id))

@app.route("/patient_information/<int:user_id>")
def patient_information_user(user_id):
    user, patient = dao.get_user_and_patient_info(user_id)
    if not user:
        return "Không tìm thấy thông tin người dùng", 404

    return render_template(
        "patient_infomation_page.html",
        current_user=user,
        patient=patient
    )
...
@app.route('/examination_form', methods=['GET'])
def get_examination_form():
    return render_template('examination_form.html')


@app.route('/examination_form', methods=['POST'])
def create_examination_form():
    ...
    # Gọi hàm add_examination_form và nhận đối tượng vừa tạo
    result = dao.add_examination_form(
        name=name,
        date=date,
        symptom=symptom,
        disease=disease,
    ...
    if result is None:
        # Nếu không có kết quả trả về, hiển thị lại form với thông báo lỗi

        return render_template('examination_form.html')

    # Nếu tạo thành công, lấy thông tin đối tượng vừa tạo
    new_examination, new_prescription = result

    flash('Đã tạo phiếu khám thành công!', 'success')

    # Tạo PDF
    pdf = dao.create_examination_pdf(new_examination, new_prescription)
    pdf.seek(0)  # Đảm bảo con trỏ ở đầu tệp

    return send_file(
        pdf,
        download_name='examination_form.pdf',  # Sửa 'attachment_filename' thành 'download_name'
        as_attachment=True,
        mimetype='application/pdf'  # Thiết lập mimetype cho header
    )
