...
def update_patient(user_id, **kwargs):
    ...
    patient = BenhNhan.query.filter_by(id=user_id).first()

    # Nếu tồn tại thì cập nhật, nếu không thì tạo mới
    if patient:
        patient.diaChi = kwargs.get('address')
        patient.soDienThoai = kwargs.get('phone')
    else:
        patient = BenhNhan(
            id=user_id,
        ...
        db.session.add(patient)  # Thêm mới nếu không tồn tại

    # Ghi lại các thay đổi vào cơ sở dữ liệu
    db.session.commit()

def get_user_and_patient_info(user_id):
...
def create_examination_pdf(new_examination, new_prescription):
    pdf_buffer = BytesIO()
    ...
    # Tiêu đề
    pdf.drawString(100, 800, "Phiếu khám")
    y_coordinate = 750

    benhNhan = NguoiDung.query.filter_by(id=new_examination.benhNhan_id).first()

    # In thông tin phiếu khám (new_examination)
    pdf.drawString(100, y_coordinate, f"Họ tên: {benhNhan.hoTen}")
    y_coordinate -= 20
    pdf.drawString(100, y_coordinate, f"Ngày giờ khám: {new_examination.ngayKham}")
    y_coordinate -= 20
    pdf.drawString(100, y_coordinate, f"Triệu chứng: {new_examination.trieuChung}")
    y_coordinate -= 20
    pdf.drawString(100, y_coordinate, f"Dự đoán bệnh: {new_examination.duDoan}")
    y_coordinate -= 20

    # In thông tin toa thuốc (new_prescription)
    pdf.drawString(100, y_coordinate, f"Thuốc: {new_prescription.thuoc.tenThuoc}")
    y_coordinate -= 20
    pdf.drawString(100, y_coordinate, f"Số lượng: {new_prescription.soLuong}")
    y_coordinate -= 20
    pdf.drawString(100, y_coordinate, f"Đơn vị: {new_prescription.lieuLuong}")
    y_coordinate -= 20
    pdf.drawString(100, y_coordinate, f"Hướng dẫn sử dụng: {new_prescription.cachDung}")

    # Lưu file PDF
    pdf.save()

    # Đảm bảo con trỏ ở đầu tệp PDF
    pdf_buffer.seek(0)
    return pdf_buffer
...
def add_examination_form(**kwargs):
    ...
    try:
        ...
        if not patient:
            ...
            return None  # Trả về None nếu không tìm thấy bệnh nhân
        ...
        if not medicine:
            ...
            return None  # Trả về None nếu không tìm thấy thuốc
        ...
        if unit:
            ...
            if not donvi_thuoc:
                ...
                return None  # Trả về None nếu đơn vị thuốc không hợp lệ
        ...
        # Trả về cả hai đối tượng nếu thành công
        return new_examination, new_prescription  # Trả về cả phiếu khám và toa thuốc
...
def lay_gia_tien(name):
    ...
    medicine_amount = QuyDinh.query.filter(func.lower(func.trim(QuyDinh.tenQuyDinh)) == name.lower().strip()).first()
    if medicine_amount:
...
def get_register_medical_by_date(date):
    # Giả sử bạn có cách lấy phiếu khám từ cơ sở dữ liệu theo ngày
    # Dưới đây là một ví dụ giả lập:
    result = db.session.query(PhieuKham).filter(PhieuKham.ngayKham == date).all()

    # Nếu có dữ liệu, định dạng lại dữ liệu để truyền vào PDF
    formatted_data = []
    for phieu in result:
        formatted_data.append({
            'date': phieu.ngayKham,
            'name': phieu.benhNhan.hoTen,
            'symptom': phieu.trieuChung,
            'disease': phieu.duDoan,
            'medicineName': phieu.toaThuoc.thuoc.tenThuoc,
            'quantity': phieu.toaThuoc.soLuong,
            'unit': phieu.toaThuoc.lieuLuong,
            'instruction': phieu.toaThuoc.cachDung,
            'avatar': phieu.benhNhan.avatar
        })
